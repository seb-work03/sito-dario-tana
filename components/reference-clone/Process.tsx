...
export function Process() {
  return (
    <section className="bg-[#5372B0] px-5 py-16 md:py-24">
      <div className="mx-auto max-w-[1536px]">
        <span className="font-mono text-sm tracking-widest text-white/60">[ OUR PROCESS ]</span>
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mt-4 mb-16">
          <h2 className="text-white font-medium text-[32px] md:text-[48px] leading-tight max-w-lg">
        ...
        <div className="hidden md:grid md:grid-cols-3 gap-6 relative">
          {steps.map((s, i) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="bg-[#5372B0] border border-white/30 rounded-2xl p-8"
              style={{ marginTop: i % 2 === 1 ? 48 : 0 }}
            >
              <div className="inline-flex items-center justify-center bg-white rounded-md w-11 h-11 mb-8">
                <span className="text-[#0D0D0D] text-sm">{s.number}</span>
              </div>
              <h3 className="text-white text-2xl md:text-3xl font-medium mb-3">{s.title}</h3>
        ...
        <div className="md:hidden flex flex-col gap-4">
          {steps.map((s, i) => (
            <motion.div
              ...
              className="bg-[#5372B0] border border-white/30 rounded-2xl p-6"
            >
              <div className="inline-flex items-center justify-center bg-white rounded-md w-9 h-9 mb-5">
                <span className="text-[#0D0D0D] text-xs">{s.number}</span>
              </div>
